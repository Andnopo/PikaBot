...
public class EasyEmbedBuilder {
    public static EmbedObject build(String color, String title, String description, String footer, IUser user) {
        return new EmbedBuilder().withColor(Color.decode(color)).withTitle(title).withDesc(description).withTimestamp(System.currentTimeMillis()).withFooterIcon(user.getAvatarURL()).withFooterText(footer).build();
    }
